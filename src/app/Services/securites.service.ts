...
export class SecuritesService {
  ...
  public updateCurrentUser(updateuserDTo: any, token: string): Observable<any> {
  
    const authHeader = `Basic ${token}`;  // Assurez-vous du préfixe d'authentification
    return this.http.put<any>(`${this.baseUrl1}/modifierMotDePasse`, updateuserDTo, {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
        'Authorization': authHeader
      })
